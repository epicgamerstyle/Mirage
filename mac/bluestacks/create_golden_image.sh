...
create_golden_image() {
    ...
    if [ -n "$adb_port" ]; then
        ...
        if $booted; then
            ...
            if [ -n "$native_ver" ]; then
                ...
                local major
                major=$(echo "$native_ver" | sed 's/^[^0-9]*//' | sed 's/[^0-9].*//')
    ...
    # Stage host-side customizations into a temp directory (small files only)
    log "Staging host-side customizations..."
    local host_staging="$OUTPUT_DIR/host"
    mkdir -p "$host_staging"

    # Promotions (custom boot loading screen)
    if [ -d "$instance_dir/Promotions" ]; then
        cp -R "$instance_dir/Promotions" "$host_staging/Promotions"
        # Strip uchg flags from the copy so it's portable
        chflags -R nouchg "$host_staging/Promotions" 2>/dev/null
        ok "Promotions (loading screen) staged"
    fi

    # AppCache (custom app icons)
    if [ -d "$instance_dir/AppCache" ]; then
        cp -R "$instance_dir/AppCache" "$host_staging/AppCache"
        ok "AppCache (app icons) staged"
    fi

    # Wallpaper and loading screen source images
    if [ -f "$SCRIPT_DIR/wallpaper.jpg" ]; then
        cp "$SCRIPT_DIR/wallpaper.jpg" "$host_staging/wallpaper.jpg"
        ok "wallpaper.jpg staged"
    fi
    if [ -f "$SCRIPT_DIR/load.jpg" ]; then
        cp "$SCRIPT_DIR/load.jpg" "$host_staging/load.jpg"
        ok "load.jpg staged"
    fi
    ...
    cat > "$OUTPUT_DIR/golden_info.json" << JSONEOF
{
    ...
    "disk_format": "qcow2",
    "includes_host_assets": true,
    "host_assets": ["Promotions", "AppCache", "wallpaper.jpg", "load.jpg"],
    "notes": "Complete golden image with Magisk, Zygisk, jorkSpoofer, custom loading screen, wallpaper, and app icons"
}
JSONEOF
    ok "Metadata saved to golden_info.json"

    # Compress directly from source (no intermediate copy of the 10GB qcow2)
    ...
    if [ -n "$sevenz" ]; then
        log "Compressing golden image with 7z (archiving directly from source - no extra copy needed)..."
        log "This may take several minutes for a ${src_size} disk image..."
        "$sevenz" a -t7z -mx=5 "$archive" \
            "$data_qcow2" \
            "$OUTPUT_DIR/golden_info.json" \
            "$host_staging" \
            >/dev/null
        local archive_size
        ...
        # Fallback: copy data.qcow2 directly (requires enough disk space)
        warn "7z not found — falling back to raw copy (needs ${src_size} free space)"
        log "Copying data.qcow2..."
    ...
    if [ -f "$archive" ]; then
        ...
        echo "  Archive: jSpoof-golden-mac-a13.7z ($final_size)"
        echo ""
    fi
    echo "  To restore from this golden image:"
    echo "  1. Extract the .7z archive"
    echo "  2. Place data.qcow2 in the target instance Engine directory"
    echo "  3. Copy host/Promotions/ and host/AppCache/ to the same Engine directory"
    echo "  4. Apply a unique jorkSpoofer profile to the new instance"
    echo "  5. The promo_guard LaunchAgent will auto-lock loading screens"
    echo ""
}

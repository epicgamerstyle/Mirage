...
public class StatusCheckActivity extends Activity {
    ...
    private final CountDownLatch browserLatch = new CountDownLatch(1);
    ...
    private String buildReport() {
        ...
        // ── Overall banner ──
        boolean allGood = nativeHealthy && tzMatch && deviceMatch && ipLookupOk && browserSyncOk;
        if (allGood) {
        ...
        sb.append("<div class='row'>");
        if (!browserSyncChecked) {
            sb.append("<div class='row-icon'>").append(checkNa).append("</div>");
            sb.append("<div class='row-body'>");
            sb.append("<div class='row-label'>App ↔ Browser</div>");
            sb.append("<div class='row-value' style='color:#4a4f54'>Could not verify</div>");
            sb.append("<div class='row-detail'>Browser probe timed out</div>");
        } else if (browserSyncOk) {
            sb.append("<div class='row-icon'>").append(checkOk).append("</div>");
            sb.append("<div class='row-body'>");
            sb.append("<div class='row-label'>App ↔ Browser</div>");
            sb.append("<div class='row-value'>Synced</div>");
            sb.append("<div class='row-detail'>OSRS and Jagex website see the same device</div>");
        } else {
            sb.append("<div class='row-icon'>").append(checkWarn).append("</div>");
            sb.append("<div class='row-body'>");
            sb.append("<div class='row-label'>App ↔ Browser</div>");
            sb.append("<div class='row-value'>Not Synced</div>");
            // Give a vague but helpful hint without exposing internals
            if (!browserModelMatch) {
                sb.append("<div class='row-detail' style='color:#e05545'>Browser identity does not match device</div>");
            } else if (!appScreenMatch) {
                sb.append("<div class='row-detail' style='color:#e05545'>Screen resolution not configured for this device</div>");
            } else if (!browserDprMatch) {
                sb.append("<div class='row-detail' style='color:#e05545'>Pixel density ratio inconsistent with display</div>");
            }
        }
        sb.append("</div></div>");
        ...
        sb.append("<button class='refresh-btn' onclick='location.reload()'>Refresh</button>");
        sb.append("</div>");
